...
#Fish swim and breed
def Fish_swim_breed(i,j,Fish,Fishmove,Shark):
...
while t <= Time:
    # move fish
    Fishmove=-1*np.ones((size,size))
    Sharkmove=-1*np.ones((size,size))
    for i in range(size):
        for j in range(size):
            if Fish[i,j] != -1:
                Fish_swim_breed(i,j,Fish,Fishmove,Shark)
    Fish[:] = Fishmove
    ...
    for i in range(size):
        for j in range(size):
            ...
            if Shark[i,j] != -1 :
                Shark_num += 1
    total_num=Fish_num+Shark_num
    Fish_number.append(Fish_num)
    Shark_number.append(Shark_num)
    total_number.append(total_num)
